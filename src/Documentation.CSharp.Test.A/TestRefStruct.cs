...
/// </remarks>
/// <typeparam name="T1">type param 1 for <see cref="TestRefStruct{T1,T2}"/></typeparam>
/// <typeparam name="T2">type param 2 after <typeparamref name="T1"/> for <see cref="TestRefStruct{T1,T2}"/></typeparam>
public readonly ref struct TestRefStruct<T1, T2>
    ...
    where T2 : class
{
    ...
    public static string Foo<T3, T4>(
        [DisallowNull] ref TestRefStruct<T1, T3> tester1,
        [NotNull] out TestRefStruct<T1, T2> tester2,
        [MaybeNull] in TestRefStruct<T4, T2> tester3)
        where T3 : class
        where T4 : unmanaged, INumber<int>
    {
        tester2 = new TestRefStruct<T1, T2>();
