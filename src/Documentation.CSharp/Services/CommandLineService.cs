...
internal class CommandLineService : BackgroundService
{
    ...
    public CommandLineService(IHostApplicationLifetime lifetime, IConfiguration configuration, ILogger<CommandLineService> logger)
    {
        Lifetime = lifetime;
        Args = configuration.GetValue<string[]>("args");
        Logger = logger;
    }
